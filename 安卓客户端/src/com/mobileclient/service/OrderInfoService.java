...
public class OrderInfoService {
	...
	public String AddOrderInfo(OrderInfo orderInfo) {
		...
		params.put("movieObj", orderInfo.getMovieObj() + "");
		params.put("moviePrice", orderInfo.getMoviePrice() + "");
		params.put("orderNum", orderInfo.getOrderNum() + "");
		params.put("orderPrice", orderInfo.getOrderPrice() + "");
	...
	public List<OrderInfo> QueryOrderInfo(OrderInfo queryConditionOrderInfo) throws Exception {
		...
		if(queryConditionOrderInfo != null) {
			...
			urlString += "&movieObj=" + queryConditionOrderInfo.getMovieObj();
		...
		try {
			...
			for (int i = 0; i < length; i++) {
				...
				orderInfo.setMovieObj(object.getInt("movieObj"));
				orderInfo.setMoviePrice((float) object.getDouble("moviePrice"));
				orderInfo.setOrderNum(object.getInt("orderNum"));
				orderInfo.setOrderPrice((float) object.getDouble("orderPrice"));
