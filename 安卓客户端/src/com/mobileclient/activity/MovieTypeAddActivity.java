...
public class MovieTypeAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.movietype_add); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("添加电影类型");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_typeName.getText().toString().equals("")) {
						Toast.makeText(MovieTypeAddActivity.this, "类型名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传电影类型信息*/
					MovieTypeAddActivity.this.setTitle("正在上传电影类型信息，稍等...");
					String result = movieTypeService.AddMovieType(movieType);
