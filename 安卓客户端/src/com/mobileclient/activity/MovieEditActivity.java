...
public class MovieEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		iv_moviePhoto.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(MovieEditActivity.this,photoListActivity.class);
			...
			public void onClick(View v) {
				try {
					...
					if(ET_movieName.getText().toString().equals("")) {
						Toast.makeText(MovieEditActivity.this, "影片名称输入不能为空!", Toast.LENGTH_LONG).show();
						ET_movieName.setFocusable(true);
						ET_movieName.requestFocus();
						return;	
					}
					movie.setMovieName(ET_movieName.getText().toString());
					if (!movie.getMoviePhoto().startsWith("upload/")) {
						//如果图片地址不为空，说明用户选择了图片，这时需要连接服务器上传图片
						MovieEditActivity.this.setTitle("正在上传图片，稍等...");
						String moviePhoto = HttpUtil.uploadFile(movie.getMoviePhoto());
						MovieEditActivity.this.setTitle("图片上传完毕！");
						movie.setMoviePhoto(moviePhoto);
					} 
					/*验证获取导演*/ 
					if(ET_director.getText().toString().equals("")) {
						Toast.makeText(MovieEditActivity.this, "导演输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_mainPerformer.getText().toString().equals("")) {
						Toast.makeText(MovieEditActivity.this, "主演输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_duration.getText().toString().equals("")) {
						Toast.makeText(MovieEditActivity.this, "时长输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_playTime.getText().toString().equals("")) {
						Toast.makeText(MovieEditActivity.this, "放映时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_price.getText().toString().equals("")) {
						Toast.makeText(MovieEditActivity.this, "票价输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_opera.getText().toString().equals("")) {
						Toast.makeText(MovieEditActivity.this, "剧情简介输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_recommendFlag.getText().toString().equals("")) {
						Toast.makeText(MovieEditActivity.this, "是否推荐输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_hitNum.getText().toString().equals("")) {
						Toast.makeText(MovieEditActivity.this, "点击率输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传电影信息*/
					MovieEditActivity.this.setTitle("正在更新电影信息，稍等...");
	...
	private void initViewData() {
		...
		this.TV_movieId.setText(movieId+"");
		for (int i = 0; i < movieTypeList.size(); i++) {
			if (movie.getMovieTypeObj() == movieTypeList.get(i).getTypeId()) {
				this.spinner_movieTypeObj.setSelection(i);
				break;
			}
		}
		this.ET_movieName.setText(movie.getMovieName());
		...
		this.ET_director.setText(movie.getDirector());
		this.ET_mainPerformer.setText(movie.getMainPerformer());
		this.ET_duration.setText(movie.getDuration());
		for (int i = 0; i < areaList.size(); i++) {
			if (movie.getAreaObj() == areaList.get(i).getAreaId()) {
				this.spinner_areaObj.setSelection(i);
				break;
			}
		}
		this.ET_playTime.setText(movie.getPlayTime());
		this.ET_price.setText(movie.getPrice() + "");
		this.ET_opera.setText(movie.getOpera());
		this.ET_recommendFlag.setText(movie.getRecommendFlag());
		this.ET_hitNum.setText(movie.getHitNum() + "");
	}
	@Override
